...
// Sample templates - these would ideally come from an API or database
...
export function PromptTemplates({ onSelectTemplate }: PromptTemplatesProps) {
  ...
  const maxScroll = templates.length * 200 - (isMobile ? 320 : 900); // Approximate max scroll width
        ...
        <div className="flex gap-3 min-w-max p-1">
          {templates.map((template) => (
            <Card
              key={template.id}
              className={cn(
                "overflow-hidden cursor-pointer min-w-[200px] max-w-[200px] transition-all duration-300 border-white/5",
            ...
            >
              <div className={cn(
                "p-4 flex flex-col gap-2 h-full bg-gradient-to-br", 
                template.color,
